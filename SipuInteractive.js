var SIPU = (function(SIPU, $, undefined) {
	...
	function do_event (ran, can, iamgelist) {
		$.each(ran,function (index) {
			$(ran[index].node).on("change mousemove input", function () {
				$(this).attr("min","1");
				$(this).attr("max","255");
				canvas_event(can[index].node, iamgelist[index], $(this).val());
			});
		});
	}

	function canvas_event (can, img, val) {
	...
	SIPU.run = function () {
		...
		do_event(input_range_list, canvas_list, image_list);
	};
	return SIPU;
})(window.SIPU || {}, jQuery);
SIPU.run();
